# bot.py — Render Web Service (free tier), polling + aiohttp health endpoint
import os
import sys
import json
import asyncio
...
if not TOKEN:
    ...
    sys.exit(1)
if CHANNEL_ID == 0:
    print("ERROR: CHANNEL_ID не задан!")
    sys.exit(1)
...
async def add_mini_task_start(message: types.Message, state: FSMContext):
    if not check_access(message.from_user.id):
        return await message.answer("Нет доступа.")
    await message.answer("Введи текст мини-задачи на сегодня:")
    await state.set_state("add_mini_task")

@dp.message_handler(state="add_mini_task")
async def add_mini_task_finish(message: types.Message, state: FSMContext):
    try:
        text = message.text.strip()
        if not text:
...
async def complete_mini_task_start(message: types.Message, state: FSMContext):
    if not check_access(message.from_user.id):
        return await message.answer("Нет доступа.")
    mini_tasks = get_mini_tasks()
    ...
    await message.answer(text)
    await state.set_state("complete_mini_task")

@dp.message_handler(state="complete_mini_task")
async def complete_mini_task_finish(message: types.Message, state: FSMContext):
    data    = await state.get_data()
    ...
    try:
        ...
        try:
            ...
            kb.add(InlineKeyboardButton("👍 Похвалить", callback_data=f"praise_{uid}_{safe_text}"))
...
# Итоги
# ======================
@dp.message_handler(lambda m: m.text and m.text.strip() == "🧐 Итоги")
...
# Планы
# ======================
@dp.message_handler(lambda m: m.text and m.text.strip() == "🎭 Планы")
...
# Статус задач
# ======================
@dp.message_handler(lambda m: m.text and m.text.strip() == "📋 Статус задач")
...
# ======================
@dp.message_handler(
    lambda m: isinstance(m.text, str) and "#отзыв" in m.text.lower(),
...
# ======================
# Фон: ежедневный опрос настроения в 21:00 МСК
# ======================
...
# ======================
# Фон: автопрожим невидимой кнопки каждые 30 минут
# ======================
...
async def on_startup_app(app):
    ...
    asyncio.create_task(dp.start_polling(reset_webhook=True, skip_updates=True))
